...
function App() {
  ...
  const handleLogin = async () => {
    if (username.trim()) {
      try {
        await axios.post(`${API_URL}/api/participants/join`, { username });
        setIsLoggedIn(true);
      } catch (error) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Audio nicht verfügbar, versuche ohne Medien:",
            audioError,
          );
      ...
      if (
        ...
        errorMessage =
          "Kamera/Mikrofon-Berechtigung verweigert. Bitte in den Browser-Einstellungen erlauben.";
      } else if (
        ...
        errorMessage =
          "Kamera/Mikrofon wird bereits von einer anderen Anwendung verwendet.";
      } else if (error.name === "NotSupportedError") {
        errorMessage =
          "WebRTC wird nicht unterstützt. Verwende HTTPS oder einen unterstützten Browser.";
      }
